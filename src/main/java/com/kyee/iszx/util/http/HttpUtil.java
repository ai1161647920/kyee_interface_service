package com.kyee.iszx.util.http;
...
import com.kyee.iszx.controller.view.ViewJump;
import com.kyee.iszx.util.log.LogService;
import com.kyee.iszx.util.string.StringUtil;

public class HttpUtil implements LogService{
    ...
    public static Result doPost(String url, Map reqParam) {
    	logger.info("请求地址：" + url);
    	logger.info("请求参数：" + reqParam);
        CloseableHttpClient httpClient = null;
        CloseableHttpResponse httpResponse = null;
        Result result = Results.newFailedResult(ErrorCodeEnum.S001);
        // 创建httpClient实例
        httpClient = HttpClients.createDefault();
        // 创建httpPost远程连接实例
        ...
        if (null != reqParam && reqParam.size() > 0) {
            ...
            Set<Entry<String, Object>> entrySet = reqParam.entrySet();
            // 循环遍历，获取迭代器
    ...
    public static String getIP(String url) {
    	...
    	try {
            ...
            ret = effectiveURI.toString() + "/"+ listStr[1];
        } catch (Exception e) {
        	logger.info("解析url异常：" + url);
            ret = "";
        }
        return ret;
    }
